...
def transition_model(corpus, page, damping_factor):
    ...
    for pages in prob:
        prob[pages]=(1-damping_factor)/len(corpus)
        if pages in links:
...
def iterate_pagerank(corpus, damping_factor):
    ...
    while True:
        new_pagerank={}
        for page in corpus:
            total=float((1-damping_factor)/len(corpus.keys()))
            for p in corpus:
                if page in corpus[p]:
                    total+=damping_factor * (pagerank[p]/len(corpus[p]))
                if not corpus[p]:
                    total+=damping_factor * (pagerank[p]/len(corpus.keys()))

            new_pagerank[page]=total

        converge = True
        for p in corpus:
            if abs(pagerank[p]-new_pagerank[p])>=0.001:
                converge=False
                break
